...
class SmeListedTab extends StatefulWidget {
  const SmeListedTab({super.key});

  @override
  State<SmeListedTab> createState() => _SmeListedTabState();
}

class _SmeListedTabState extends State<SmeListedTab> {
  ...
  Future<void> _fetchIposFromApi() async {
    ...
    try {
      // Fetch IPOs from API with type 'sme' and category 'listed'
      ...
      if (FirebaseService.isAvailable) {
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} listed SME IPOs (Firebase not available)',
        );
      }
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search listed SME IPOs...',
          hintStyle: TextStyle(
  ...
  Widget _buildIposList() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              _searchQuery.isNotEmpty ? 'No IPOs found' : 'No listed SME IPOs',
              style: Theme.of(context).textTheme.titleLarge,
            ),
            const SizedBox(height: 8),
            Text(
    ...
    return Padding(
      ...
      child: IpoDataTable(
        key: ValueKey('sme_listed_${_filteredIpos.length}'),
        ipos: _filteredIpos,
        ipoType: 'sme',
        category: 'listed',
        onEdit: (ipo) {
