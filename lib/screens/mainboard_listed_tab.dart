...
class MainboardListedTab extends StatefulWidget {
  const MainboardListedTab({super.key});

  @override
  State<MainboardListedTab> createState() => _MainboardListedTabState();
}

class _MainboardListedTabState extends State<MainboardListedTab> {
  ...
  Future<void> _fetchIposFromApi() async {
    ...
    try {
      // Fetch IPOs from API with type 'mainboard' and category 'listed'
      ...
      if (FirebaseService.isAvailable) {
        ...
        for (int index = 0; index < ipos.length; index++) {
          ...
          try {
            ...
            final existingIpo = await FirebaseService.findIpoInManagement(
              companyId: ipo.id ?? 0,
              ipoType: 'mainboard',
              category: 'listed',
            );
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} IPOs. Added: $successCount, Skipped: $skippedCount, Failed: $failedCount',
        );
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} listed Mainboard IPOs (Firebase not available)',
        );
      }
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search listed Mainboard IPOs...',
          hintStyle: TextStyle(
  ...
  Widget _buildIposList() {
    ...
    return Padding(
      ...
      child: IpoDataTable(
        key: ValueKey('mainboard_listed_${_filteredIpos.length}'),
        ipos: _filteredIpos,
        ipoType: 'mainboard',
        category: 'listed',
        onEdit: (ipo) {
