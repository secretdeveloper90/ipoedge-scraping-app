...
class SmeUpcomingTab extends StatefulWidget {
  const SmeUpcomingTab({super.key});

  @override
  State<SmeUpcomingTab> createState() => _SmeUpcomingTabState();
}

class _SmeUpcomingTabState extends State<SmeUpcomingTab> {
  ...
  Future<void> _fetchIposFromApi() async {
    ...
    try {
      // Fetch IPOs from API with type 'sme' and category 'upcoming'
      ...
      if (FirebaseService.isAvailable) {
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} upcoming SME IPOs (Firebase not available)',
        );
      }
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search upcoming SME IPOs...',
          hintStyle: TextStyle(
  ...
  Widget _buildIposList() {
    ...
    return Padding(
      ...
      child: IpoDataTable(
        key: ValueKey('sme_upcoming_${_filteredIpos.length}'),
        ipos: _filteredIpos,
        ipoType: 'sme',
        category: 'upcoming',
        onEdit: (ipo) {
