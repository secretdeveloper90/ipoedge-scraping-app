...
class MainboardUpcomingTab extends StatefulWidget {
  const MainboardUpcomingTab({super.key});

  @override
  State<MainboardUpcomingTab> createState() => _MainboardUpcomingTabState();
}

class _MainboardUpcomingTabState extends State<MainboardUpcomingTab> {
  ...
  Future<void> _fetchIposFromApi() async {
    ...
    try {
      // Fetch IPOs from API with type 'mainboard' and category 'upcoming'
      ...
      if (FirebaseService.isAvailable) {
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} upcoming Mainboard IPOs (Firebase not available)',
        );
      }
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search upcoming Mainboard IPOs...',
          hintStyle: TextStyle(
  ...
  Widget _buildIposList() {
    ...
    return Padding(
      ...
      child: IpoDataTable(
        key: ValueKey('mainboard_upcoming_${_filteredIpos.length}'),
        ipos: _filteredIpos,
        ipoType: 'mainboard',
        category: 'upcoming',
        onEdit: (ipo) {
