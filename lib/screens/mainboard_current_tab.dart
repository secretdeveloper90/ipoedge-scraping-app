...
class MainboardCurrentTab extends StatefulWidget {
  const MainboardCurrentTab({super.key});

  @override
  State<MainboardCurrentTab> createState() => _MainboardCurrentTabState();
}

class _MainboardCurrentTabState extends State<MainboardCurrentTab> {
  ...
  Future<void> _fetchIposFromApi() async {
    ...
    try {
      // Fetch IPOs from API with type 'mainboard' and category 'live'
      ...
      if (FirebaseService.isAvailable) {
        ...
        SnackbarUtils.showSnackBar(
          context,
          'Fetched ${ipos.length} Mainboard IPOs (Firebase not available)',
        );
      }
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Search Mainboard IPOs...',
          hintStyle: TextStyle(
  ...
  Widget _buildIposList() {
    ...
    return Padding(
      ...
      child: IpoDataTable(
        key: const ValueKey('mainboard_live_table'),
        ipos: _filteredIpos,
        ipoType: 'mainboard',
        category: 'live',
        onEdit: (ipo) {
